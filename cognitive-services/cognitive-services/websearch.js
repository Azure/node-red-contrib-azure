...
module.exports = function(RED)
{
    function websearch(config)
    {
        ...
        this.on('input', function(msg)
        {
            ...
            else
            {
                ...
                {
                    var options = {
                        url: 'https://api.cognitive.microsoft.com/bing/v5.0/search?q=' + encodeURI(msg.payload),
                        method: 'GET',
                        headers: {
                    ...
                    request(options, function (error, response, body)
                    {
                        try
                        {
                            if (!error)
                            {
                                ...
                                if (response.statusCode == 200 && body != null && body.webPages != null && body.webPages.value != null)
                                {
                                     var urls = [];
                                     for (var i = 0; i < body.webPages.value.length; i++)
                                     {
                                         urls[i] = body.webPages.value[i].url;
                                     }
                ...
                {
                    var options = {
                        url: 'https://api.cognitive.microsoft.com/bing/v5.0/search/trendingtopics',
                        method: 'GET',
                        headers: {
                            'Ocp-Apim-Subscription-Key': this.credentials.key
                        }
    ...
    RED.nodes.registerType("Bing Web Search", websearch,
    {
        credentials: {
            key: {
                type: "password"
